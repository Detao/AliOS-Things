...
#define PRODUCT_KEY      "a1kNJHrmLPe"
#define PRODUCT_SECRET   "QO856W74sJq8NErE"
#define DEVICE_NAME      "device1"
#define DEVICE_SECRET    "K6fc5U3D0HdsoTfMXB05BLz1lNTzLdRq"
#else
...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    property_payload = aos_malloc(100);
    ...
    root = cJSON_Parse(request);
    item_PowerSwitch = cJSON_GetObjectItem(root,"PowerSwitch_1");
    printf("\r\n Turn on power \r\n");
    if(item_PowerSwitch != NULL ){
        if(item_PowerSwitch->valueint){
            IR05F_Send();
            printf("\r\n Turn on power \r\n");
            sprintf(property_payload,"{\"WindSpeed_1\":%d,\"TargetTemperature\":%d,\"WorkMode_1\":%d}",5,16,1);
        }else{
            IR05F_Send();
            printf("\r\n Turn off power\r\n");
            sprintf(property_payload,"{\"WindSpeed_1\":\" \",\"TargetTemperature\":\" \",\"WorkMode_1\":\" \"}");
        }
    }
...
void user_post_property(void)
{
    ...
    /* Normal Example */
    sprintf(property_payload,"{\"CurrentTemperature\":%d,\"CurrentHumidity\":%d}",temperature,humidity);
...
int linkkit_main(void *paras)
{
    ...
    while (1) {
        ...
        if(time_now_sec - time_prev_sec > UPLOAD_SPACE){
            user_post_property();
